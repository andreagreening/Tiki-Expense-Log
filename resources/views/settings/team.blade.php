						...
						<div class="list-group">
    						@foreach($invitesRecieved as $invite) 
    							<div class="list-group-item">
    								...
    								<a href="{{ route('team.denyInvite', $invite) }}" class="custom-danger pull-right">Deny</a>
    								<a href="{{ route('team.acceptInvite', $invite->token) }}" class="custom-success pull-right margin-right-10">Accept</a>
    							</div>
    						@endforeach	
						</div>
				</div>
            ...
            @if(!$invitesSent->isEmpty())
                <div class="panel panel-default border-primary">
                    ...
                    @foreach($invitesSent as $inviteSent)
                            <div class="list-group-item">
                                {{ $inviteSent->email }}
                                <a href="{{ route('team.deleteInvite', $inviteSent->id) }}" class="custom-info pull-right">Cancel</a>
                            </div>
                    @endforeach
                    </div>
                </div>
